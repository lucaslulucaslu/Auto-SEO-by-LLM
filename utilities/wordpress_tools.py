...
def update_summary_qa(post_ID, content):
    ...
    system_prompt = """你是美国留学领域的专家。用户将输入一段关于美国留学相关的文章，请你根据该文章内容提出5个读者可能会感兴趣的问题。

        针对每个问题，请提供一个【简短精炼】的回答。要求如下：
        1. 紧贴文章内容：回答必须完全基于提供的文章，直接提炼核心信息。
        2. 禁止使用列表：严禁使用任何形式的分点、 bullet points 或编号。
        3. 段落化输出：每个回答应仅由1-2个简洁的自然段组成，字数控制在150字以内。
        4. 语言风格：专业、直接、无废话。
